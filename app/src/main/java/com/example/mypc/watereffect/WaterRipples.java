...
public class WaterRipples extends GdxLWP implements InputProcessor {
    ...
    static final float DAMPING = 0.7f;
    static final float DISPLACEMENT = -7;
    static final float TICK = 0.099f;
    ...
    Vector3 point_common = new Vector3();
    float[][] last;
    float[][] curr;
    float[][] intp;
    float[] vertices;
        ...
        while (accum > TICK) {
          //  for (int i = 0; i < 5; i++) {
              /*  if (Gdx.input.isTouched(i)) {
                    Ray ray1 = camera.getPickRay(Gdx.input.getX(i), Gdx.input.getY(i));
                    Intersector.intersectRayPlane(ray1, plane, point);
                    touchWater(point);
                }*/

                ray = camera.getPickRay(random.nextInt(Gdx.graphics.getWidth()),random.nextInt(Gdx.graphics.getHeight()));
